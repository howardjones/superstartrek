...
1990 IF S+E>10 THEN IF E>10 OR D(7)=0 THEN GOTO 1060
...
2080 FOR I=1 TO 9 : IF LEFT$(A$,3)<>MID$(A1$,3*I-2,3) THEN GOTO 1160
...
2310 IF C1>=1 AND C1<9 THEN GOTO 1350
2330 PRINT "   LT. SULU REPORTS, 'INCORRECT COURSE DATA, SIR!'" : GOTO 1990

2350 X$ = "8" : IF D(1)<0 THEN X$ = "0.2"
2360 PRINT "WARP FACTOR (0-";X$;")"; : INPUT W1 : IF D(1)<0 AND W1>.2 THEN GOTO 1470
2380 IF W1>0 AND W1<=8 THEN GOTO 1490
...
2490 N=INT(W1*8+.5) : IF E-N>=0 THEN GOTO 1590
...
2590 FOR I=1 TO K3 : IF K(I,3)=0 THEN GOTO 1700
...
3170 FOR I=1TON : S1=S1+X1 : S2=S2+X2 : IF S1<1 OR S1>=9 OR S2<1 OR S2>=9 THEN GOTO 1500
3240 S8=INT(S1)*24+INT(S2)*3-26 : IF MID$(Q$,S8,2)="  " THEN GOTO 1360
...
3450 T=T+T8 : IF T>T0+T9 THEN GOTO 1220
...
3850 IF T>T0+T9 THEN GOTO 1220
3860 IF 8*Q1+Q2=8*Q4+Q5 THEN GOTO 1370
...
4400 IF E-X<0 THEN GOTO 1360
4410 E=E-X : IF D(7)<0 THEN X=X*RND(1)
4450 H1=INT(X/K3) : FOR I=1 TO 3 : IF K(I,3)<=0 THEN GOTO 1670
4480 H=INT((H1/FND(0))*(RND(1)+2)) : IF H>.15*K(I,3) THEN GOTO 1530
...
4650 K(I,3)=0 : G(Q1,Q2)=G(Q1,Q2)-100 : Z(Q1,Q2)=G(Q1,Q2) : IF K9<=0 THEN GOTO 1370
...
4780 IF C1>=1 AND C1<9 THEN GOTO 1850
...
4960 IF X3<1ORX3>8ORY3<1ORY3>8 THEN GOTO 1490
5000 PRINT "               ";X3;",";Y3 : A$ = "   " : Z1=X : Z2=Y : GOSUB 8830
5050 IF Z3<>0 THEN GOTO 1920
5060 A$ = "+K+" : Z1=X : Z2=Y : GOSUB 8830 : IF Z3=0 THEN GOTO 1210
5110 PRINT "*** KLINGON DESTROYED ***" : K3=K3-1 : K9=K9-1 : IF K9<=0 THEN GOTO 1370
5150 FOR I=1 TO 3 : IF X3=K(I,1) AND Y3=K(I,2) THEN GOTO 1190
5180 NEXT I : I=3
5190 K(I,3)=0 : GOTO 5430
5210 A$ = " * " : Z1=X : Z2=Y : GOSUB 8830 : IF Z3=0 THEN GOTO 1280
5260 PRINT "STAR AT";X3;",";Y3;"ABSORBED TORPEDO ENERGY." : GOSUB 6000 : GOTO 1990

5280 A$ = ">!<" : Z1=X : Z2=Y : GOSUB 8830 : IF Z3=0 THEN GOTO 1760
5330 PRINT "*** STARBASE DESTROYED ***" : B3=B3-1 : B9=B9-1
5360 IF B9>0ORK9>T-T0-T9 THEN GOTO 1400
...
6290 PRINT : PRINT : IF B9=0 THEN GOTO 1360
...
6450 IF INT(I+.5)<1 OR INT(I+.5)>8 OR INT(J+.5)<1 OR INT(J+.5)>8 THEN GOTO 1540
6490 A$ = ">!<" : Z1=I : Z2=J : GOSUB 8830 : IF Z3=1 THEN GOTO 1580
...
7560 O1$ = "     ----- ----- ----- ----- ----- ----- ----- -----"
7570 PRINT O1$ : FOR I=1 TO 8 : PRINT I; : IF H8=0 THEN GOTO 1740
...
7970 X$ = "S" : IF B9<2 THEN X$ = "" : IF B9<1 THEN GOTO 1010
...
8220 X=X-A : A=C1-W1 : IF X<0 THEN GOTO 1350
8250 IF A<0 THEN GOTO 1410
8260 IF X>0 THEN GOTO 1280
8270 IF A=0 THEN C1=5 : GOTO 8290
8280 C1=1
8290 IF ABS(A)<=ABS(X) THEN GOTO 1330
...
8410 C1=7
8420 IF ABS(A)>=ABS(X) THEN GOTO 1450
...
8590 R1=FNR(1) : R2=FNR(1) : A$ = "   " : Z1=R1 : Z2=R2 : GOSUB 8830 : IF Z3=0 THEN GOTO 1590
8600 RETURN
